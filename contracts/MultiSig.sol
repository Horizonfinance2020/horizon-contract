...
contract MultiSig{
  ...
  function init(address[] memory s) public enough_signers(s){
    require(owner == msg.sender, "only for owner");
    require(is_inited == false, "already init");
    is_inited = true;
  ...
  //@sender is no use anymore, however we keep it for backward-compatibilty
  function update_and_check_reach_majority(uint64 id, string memory name, bytes32 hash, address sender) public only_multisig_proxy returns (bool){
    ...
    if(!is_all_minus_sig(minority, id, name, hash, tx.origin))
      return false;
    set_called(hash);
    return true;
  }
