...
async function performMigration(deployer, network, accounts) {
  ...
  console.log("creating for 2 week yusd market");
  ...
  if(network.includes("main")){
    ...
    await gatekeeper.resetSupportRatios([191780, 383561, 575342, 767123, 958904, 1150684, 1342465, 1534246, 1726027, 1917808, 2109589, 2301369, 2493150, 2684931, 2876712, 3068493, 3260273, 3452054, 3643835, 3835616])
    console.log("setting yield interest pool")
    await gatekeeper.changeYieldPool(fee_pool_addr);
  }

  const wdata = JSON.stringify(brief);
  await fs.writeFile(network+'-15-2week-yusd.json', wdata, (err) => {
    if (err) {
        throw err;
    }
